...
$registerResponse = curl.exe -X POST "https://afrimercato-backend-1.onrender.com/api/auth/register" `
...
$resendResponse = curl.exe -X POST "https://afrimercato-backend-1.onrender.com/api/auth/resend-verification" `
  -H "Content-Type: application/json" `
...
$locationResponse = curl.exe -X GET "https://afrimercato-backend-1.onrender.com/api/vendors/search-by-location?lat=40.7128&lon=-74.0060&radius=10" -s
...
Write-Host '   curl.exe -X GET "https://afrimercato-backend-1.onrender.com/api/vendor/dashboard/earnings" -H "Authorization: Bearer $token"' -ForegroundColor Gray
