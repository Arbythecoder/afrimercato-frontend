...
function OrderDetail() {
  ...
  // ✅ CANCELLATION HANDLER – now uses the orderAPI service (POST)
  const handleCancelOrder = async () => {
    if (!order) return
    if (!window.confirm('Are you sure you want to cancel this order?')) return

    try {
      // Use the existing orderAPI.cancel – this sends a POST request
      // (after we changed the method in services/api.js from PATCH to POST)
      const response = await orderAPI.cancel(order._id)

      if (response.success) {
        // Refresh order data to reflect the new 'cancelled' status
        await fetchOrder()
  ...
  // Smart mapping for the 5-step customer progress bar
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              {['pending', 'confirmed'].includes(order.status) && (
                // Cancel button now uses the API service (via handleCancelOrder)
                <button
              ...
              {/* Tracking button for delivery states */}
