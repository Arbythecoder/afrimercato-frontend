...
function OrderStatusTimeline({ order }) {
  ...
  const timelineStages = [
    ...
    },
    {
      ...
      label: 'Packing Order',
      icon: '📦',
      description: 'Items packed and sealed',
    },
    {
    ...
    },
    {
      ...
      icon: '🏍️',
      description: 'Rider matched to order',
    },
    {
      status: 'picked_up_by_rider',
      label: 'Picked Up',
      icon: '🛍️',
      description: 'Rider collected the order',
    },
    {
  ...
  // Helper to determine the "Weight" or "Index" of a status in the timeline
  const getStatusWeight = (status) => {
    return timelineStages.findIndex(s => s.status === status)
  }

  const currentStageIndex = getStatusWeight(order.status)

  // Determine the status of an individual stage node
  const getStageStatus = (stageIndex) => {
    if (isCancelled) return 'cancelled'

    // If the order jumped from 'confirmed' -> 'packed', the picking steps were bypassed.
    const isPickerStep = ['assigned_to_picker', 'picking'].includes(timelineStages[stageIndex].status)
    const hasPassedPickerPhase = currentStageIndex >= getStatusWeight('packed')

    // Check if the timeline actually has history for this step
    const hasHistory = (order.statusHistory || []).some(h => h.status === timelineStages[stageIndex].status)

    if (hasPassedPickerPhase && isPickerStep && !hasHistory) {
      return 'skipped' // New state for bypassed steps!
    }
    ...
    switch (status) {
      ...
      case 'skipped':
        return {
          ...
          line: 'bg-[#00897B]', // The line continues through skipped steps to maintain connection
        }
      case 'cancelled':
        return {
          bg: 'bg-red-500',
  ...
  useEffect(() => {
    // Calculate progress based on the current stage index (even if steps were bypassed)
    const targetProgress = currentStageIndex === -1 ? 0 : ((currentStageIndex + 1) / timelineStages.length) * 100
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200 animate-fadeIn">
          ...
          <div className="flex justify-between mt-2 text-xs text-gray-500">
            ...
            <span className="font-semibold text-[#FFB300]">
              {timelineStages[currentStageIndex]?.label || 'Pending'}
            </span>
            <span>Completed</span>
          </div>
        </div>
                ...
                >
                  ...
                  {!isLast && (
                    <div className="absolute left-5 top-12 w-0.5 h-full">
                      <div
                        className={`w-full transition-all duration-500 ${colors.line}`}
                        style={{
                          height: (stageStatus === 'completed' || stageStatus === 'skipped') ? '100%' : stageStatus === 'current' ? '50%' : '0%'
                        }}
                      />
                    </div>
                  )}
                  ...
                  <div className="relative z-10 flex-shrink-0">
                    <div
                      className={`
                        ...
                        ${stageStatus === 'current' ? 'animate-pulseGlow scale-110 ring-4 ring-[#FFB300] ring-opacity-50' : ''}
                        ${stageStatus === 'completed' ? 'animate-bounceIn' : ''}
                        ${stageStatus === 'upcoming' || stageStatus === 'skipped' ? 'scale-90 opacity-50' : ''}
                      `}
                    >
                      <span className={`text-xl ${stageStatus === 'skipped' ? 'text-gray-400' : 'text-white'}`}>
                        {stageStatus === 'skipped' ? '⏭' : stage.icon}
                      </span>
                    </div>
                    {stageStatus === 'completed' && (
                      <div className="absolute -top-1 -right-1 w-5 h-5 bg-[#00897B] rounded-full flex items-center justify-center animate-checkmark">
                  ...
                  <div className="flex-1 pt-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {stageStatus === 'skipped' && (
                        <span className="text-sm text-gray-400 italic">Bypassed</span>
                      )}
                      {stageStatus === 'current' && (
                        <span className="px-3 py-1 bg-[#FFB300] text-gray-900 text-xs font-semibold rounded-full">
                          Current
                    ...
                    <p className={`text-sm ${stageStatus === 'skipped' ? 'text-gray-400' : 'text-gray-600'}`}>
                      {stageStatus === 'skipped' ? 'Skipped via Fast-Track' : stage.description}
                    </p>
                    {stageStatus === 'current' && (
                      <div className="mt-2 p-3 bg-yellow-50 border-l-4 border-[#FFB300] rounded">
                        <p className="text-sm text-gray-700">
          ...
          {displayedHistory.map((entry, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {entry.user && (
                  <p className="text-xs text-gray-500 mt-1">By {entry.user.name || entry.user.firstName || 'System'}</p>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5 shadow-md border border-green-200">
          ...
          <p className="text-2xl font-bold text-green-700">
            {Math.round(currentStageIndex === -1 ? 0 : ((currentStageIndex + 1) / timelineStages.length) * 100)}%
          </p>
          <p className="text-sm text-gray-600 mt-1">Order completion</p>
        </div>
      ...
      <style jsx>{`
        ...
        @keyframes pulseGlow {
          0%, 100% { box-shadow: 0 0 20px rgba(255, 179, 0, 0.5); }
          50% { box-shadow: 0 0 40px rgba(255, 179, 0, 0.8); }
        }
        @keyframes bounceIn {
